    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex-1 px-4 py-3 overflow-y-auto space-y-2">
          ...
          {messages.map((m) => (
            <div key={m.id || `${m.timestamp}-${m.from_user_id || ''}`} className="flex flex-col">
              <div className="text-[11px] text-gray-500 mb-0.5">
                {new Date(m.timestamp || m.ts || Date.now()).toLocaleTimeString()}
              </div>
              <div className="inline-block bg-gray-700/80 text-sm text-gray-100 px-3 py-2 rounded-lg max-w-xs">
                {m.message}
              </div>
            </div>
          ))}
